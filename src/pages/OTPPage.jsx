...
function OTPPage({ email, onGoBack, onRegistrationComplete, onResendSuccess, registeredData }) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (!registeredData) {
        throw new Error('No registration data available. Please register again.')
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      ...
      <h1 className="mb-3">Don't get a code? <span><button type="button" onClick={handleResendOtp} className="text-[#003F66] font-bold hover:underline">Resend</button></span></h1>
      <Button type="submit" disabled={isVerifying} className='bg-[#003F66] w-full'>{isVerifying ? "Verifying..." : "Verify"}</Button>
      </form>
    </div>
  )
}
